var mongoose = require('mongoose')
  ...
  , coineal = require('./markets/coineal');
...
function get_market_data(market, cb) {
  switch (market) {
    ...
    case 'coinexchange':
      ...
      break;
    case 'coineal':
      coineal.get_data(settings.markets.coin, settings.markets.exchange, function (err, obj) {
        return cb(err, obj);
      });
      break;
    default:
...
module.exports = {
  ...
  get_tx_between_addresses: function (adr1, adr2, cb) {
    var ab = [];
    var ba = [];
    Tx.find(
      {
        $and: [
          {"vout.addresses": {$all: [adr2]}},
          {"vin.addresses": {$all: [adr1]}}
        ]
      }, function (err, txes) {
        Tx.find(
          {
            $and: [
              {"vout.addresses": {$all: [adr1]}},
              {"vin.addresses": {$all: [adr2]}}
            ]
          }, function (err, txes1) {
            return cb(txes, txes1);
          });
      });
  },
  get_address_txes: function (adr, cb) {
    Tx.find(
      {
        $or: [
          {"vout.addresses": {$all: [adr]}},
          {"vin.addresses": {$all: [adr]}}
        ]
      }, function (err, txes) {
        return cb(txes);
      });
  ...
  get_address_vin_txes: function (adr, cb) {
    Tx.find(
      {
        $and: [
          {"vin.addresses": {$ne: adr}},
          {"vout.addresses": {$all: [adr]}}
        ]
      }, function (err, txes) {
        return cb(txes);
  ...
  is_tx_amount_in_range: function(tx, hash, from, to, cb) {
    let vi = 0;
    let vo = 0;
    for (let vin_key in tx.vin) {
      let v = tx.vin[vin_key];
      if (v.addresses === hash) {
        vi = v.amount;
        break;
      }
    }
    for (let vout_key in tx.vout) {
      let v = tx.vout[vout_key];
      if (v.addresses === hash) {
        vo = v.amount;
        break;
      }
